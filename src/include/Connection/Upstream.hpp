...
namespace Hermes
{
    ...
    {
        virtual void Run() = 0;
        virtual void Enable(const UpstreamSettings&) = 0;

        virtual void Signal(unsigned sessionId, const ServiceDescriptionData&) = 0;
        virtual void Signal(unsigned sessionId, const MachineReadyData&) = 0;
        virtual void Signal(unsigned sessionId, const RevokeMachineReadyData&) = 0;
        virtual void Signal(unsigned sessionId, const StartTransportData&) = 0;
        virtual void Signal(unsigned sessionId, const StopTransportData&) = 0;
        virtual void Signal(unsigned sessionId, const QueryBoardInfoData&) = 0;
    ...
    struct UpstreamDeleter {
        void operator()(IUpstream* ptr) { ptr->Delete(); }
    };

    template<typename T>
    using UpstreamPtrT = std::unique_ptr<T, UpstreamDeleter>;

    typedef UpstreamPtrT<IUpstream> UpstreamPtr;
